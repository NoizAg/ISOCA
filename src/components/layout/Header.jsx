...
export default function Header() {
  ...
  const mobileMenuPortal = mobileOpen && typeof window !== 'undefined' && document.body
    ? ReactDOM.createPortal(
        <>
          ...
          <div
            style={{ zIndex: 9998 }}
            className={`fixed inset-0 bg-black bg-opacity-50 transition-opacity duration-300 ${mobileOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
          ...
          <nav
            style={{ zIndex: 9999 }}
            className={`fixed top-0 right-0 h-full w-[85vw] max-w-sm bg-white shadow-2xl transform transition-transform duration-300 ease-in-out ${
  ...
  return (
    <header className="sticky top-0 z-60 bg-white/95 backdrop-blur-md shadow-lg border-b border-gray-100">
